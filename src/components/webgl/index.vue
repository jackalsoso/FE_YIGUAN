<template>
  <div>
    <!-- <button v-on:click="insert({id:'12',frame_id:'',url:'/resource/01.jpg',type:'0',index:'023',backcolor:0xFFFFFF})"  style="width: 60px;height:40px;position: absolute">添加画框</button>
    ...
    <button v-on:click="exhibition_style('hk1_1')"  style="width: 60px;height:40px;position: absolute;left:240px">调换位置</button>
    <button v-on:click="set_origin_position('023')"  style="width: 60px;height:40px;position: absolute;left:300px">隐藏层</button>
    <button v-on:click="delete_all_frame()"  style="width: 60px;height:40px;position: absolute;left:240px">删除全部</button> -->
    <!-- <button v-on:click="gofloor('2')"  style="width: 60px;height:40px;position: absolute;right:0px">隐藏层</button> -->
...
export default {
  ...
  props: ["LoadScene", "sceneLoad", "mouseClick", "deleteAllTrue","gellaryLoad"],
  created: function() {
  ...
  methods: {
    ...
    dispose:function(parent,child){
        ...
        if(child instanceof THREE.Mesh||child instanceof THREE.Line){
          if(child.material.type=="MeshPhongMaterial"){
            ...
            for(var i=child.material.length;i>0;i--){
              child.material[i-1].dispose();
            }
          }
    ...
    scene_load: function(scene) {
      ...
      scene.traverse(function(child) {
        if (child instanceof THREE.Mesh) {
          ...
          } else if (child.name.startsWith("interactive")) {
            //bottom
      ...
      this.view_position = scene.getObjectByName("view_position");
      top = scene.getObjectByName("top");

      for (var i = 0; i < this.view_position.children.length; i++) {
        var sphereGeometry = new THREE.SphereGeometry(100, 10, 10);
        var phongMaterial = new THREE.MeshPhongMaterial({
          shininess: 5,
          emissiveIntensity:1,
          emissive:0xffff66
        });
        var point = new THREE.Mesh(sphereGeometry, phongMaterial);
        point.castShadow = true;
        point.receiveShadow = true;
        let vec3 = this.view_position.children[i].position;
        point.position.set(vec3.x, vec3.z, -vec3.y);
        point.name = this.view_position.children[i].name;
        this.scene.add(point);
      }

      if (this.$props.gellaryLoad) {
        this.$props.gellaryLoad();
      }
    },
    ...
    dblclick: function(e) {
      ...
      if (intersects.length != 0) {
        if(intersects[0].object.name.startsWith("interactive")){   //bottom   interactive
        ...
        if(intersects[0].object.name.startsWith("P_")){
          var str = intersects[0].object.name.substring(2);
          var value=Number(str);
          value=this.prefix_integer(value-35,3);
          this.set_origin_position(value.toString());
        }
      }
    },
    click: function(e) {
      ...
      if (nowTime - lastClickTime < 400) {
        ...
        clickTimer = setTimeout(() => {
          if (this.$props.mouseClick) {
            ...
            // debugger
            if (intersects.length != 0) {
              this.$props.mouseClick(intersects[0].object.parent.realName);
            }
          }
        }, 400);
      }
    ...
    delete_frame: function(id) {
      let that = this;
      this.frame_container.children.some(function(child) {
        if (Object.is(child.realName, id)) {
          console.log(child.name);
          that.frame_container.remove(child);
          return true;
        }
      });
    },
    delete_all_frame: function() {
      let that = this;
      for (var i = this.frame_container.children.length; i > 0; i--) {
        that.frame_container.remove(that.frame_container.children[i - 1]);
      }
      if (this.$props.deleteAllTrue) {
        this.$props.deleteAllTrue();
      }
      return true;
    },
    set_origin_position: function(index) {
      let position = new THREE.Vector3(0, 0, 0);
      let point = this.frame_position.getObjectByName("P_" + index);
      point.getWorldPosition(position);
      ...
      let look = point.localToWorld(new THREE.Vector3(0, -3000, 0));
      this.camera.position.set(look.x, this.camera.position.y, look.z);
      this.camera.lookAt(position);
    },
    set_origin_position2: function(index) {
      // debugger
      let position = new THREE.Vector3(0, 0, 0);
      let point = this.frame_position.getObjectByName("P_" + index);
      point.getWorldPosition(position);
      ...
      let look = point.localToWorld(new THREE.Vector3(0, -3000, 0));
      this.camera.position.set(look.x, this.camera.position.y+3500, look.z);
      this.camera.lookAt(position);
    },
    gofloor(index){
      var index1=1;
      ...
      let position = new THREE.Vector3(0, 0, 0);
      let point = this.frame_position.getObjectByName("P_" + index1);
      point.getWorldPosition(position);
      ...
      let look = point.localToWorld(new THREE.Vector3(0, -3000, 0));
     if(index == 1){
      this.camera.position.set(look.x, 1800, look.z);
     }else{
      this.camera.position.set(look.x, 5500, look.z);
      }
      this.camera.lookAt(position);
    },
    exhibition_style: function(id) {
      this.clear_style();
      if (this.frame_kind.has(id) && style_point) {
        let child = this.frame_kind.get(id).clone();

        this.style_container.add(child);
        child.name = "style_child";

        let shadow = child.getObjectByName("shadow");
        shadow.material.transparent = true;

        let position = new THREE.Vector3(0, 0, 0);
        style_point.getWorldPosition(position);
        child.position = position;
        let look = style_point.localToWorld(new THREE.Vector3(0, 100, 0));
        child.lookAt(look);
      }
    },
    ...
    clear_style: function() {
      let child = this.style_container.getObjectByName("style_child");
      if (child) {
        this.style_container.remove(child);
      }
    },
  },
};
